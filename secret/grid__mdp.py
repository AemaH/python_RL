...
class Grid_Mdp:
    ...
    def gen_randompi_sample(self, num):
        ...
        key = '%d_%s'%(state, action);
    #定义判定 动作_状态，然后用以输出相关过程
    #也就是输出 看一步步是怎么走的，不在上面那些情况的
    #也只有到了 6 7 8，或者撞墙壁了，只要输出当前的状态就好
        if key in self.t: 
            next_state = self.t[key]; 
        else:
            next_state = state       
        ...
        is_terminal = False
        #有输出self.t 说明可以判定其不是最终状态
        if next_state in self.terminal_states:
            is_terminal = True
      #如果下一个状态在self.terminal_states说明是最终状态
        if key not in self.rewards:
            r = 0.0
        else:
            r = self.rewards[key];
         #如果回报的key没有在self.rewards说明没有回馈
         #也还是0
        return is_terminal, next_state, r;
